...
// 1% chance = 3.6deg, 0.69% chance = 2.484deg. There is NO 25%/90deg floor.
const oldWinDegrees = "const winDegrees = Math.max(90, Math.min(360, shownChance * 3.6));";
const newWinDegrees = "const winDegrees = Math.min(360, Math.max(0, shownChance) * 3.6);";
if (fixed.includes(oldWinDegrees)) {
  fixed = fixed.replaceAll(oldWinDegrees, newWinDegrees);
}

if (fixed.includes("Math.max(90, Math.min(360, shownChance * 3.6))")) {
  throw new Error("Upgrade chance guard: visual chance sector still has the old 90deg/25% floor");
}
...
if (fixed !== source) {
  ...
  const valid = source.includes("const MIN_CHANCE = 0.01;") && source.includes("target > 0 ? (input / target) * 100 : 0") && source.includes("const winDegrees = Math.min(360, Math.max(0, shownChance) * 3.6);");
  if (!valid) throw new Error("Upgrade chance guard: expected precise chance formula was not found");
