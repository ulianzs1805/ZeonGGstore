...
# Keep radial burst, but simplify the win phase: shards are created directly inside the left slot.
...
s = re.sub(r'<UpgradeFragmentLayer[^>]*/>\s*', '', s, count=1)
s = re.sub(r'( particles=\{particles\})(?: fragmentItem=\{phase === "burst" \? rightFragments : null\} fragmentMode=\{phase === "burst" \? "burst" : null\} particles=\{particles\})+', r'\1', s)
...
p.write_text(s)
print('simplified winning shards: appear directly in left slot')
