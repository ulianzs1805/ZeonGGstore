        ...
        {!opened ? (
          <>
            <div className={`relative mx-auto mt-7 h-60 w-60 transition duration-500 ${opening ? "scale-110 -translate-y-2" : ""}`}>
              <Image src="/cases/CaseRecoceryUpgrade.jpeg" alt="Кейс отыгрыша" fill className="object-contain drop-shadow-[0_0_45px_rgba(124,58,237,.5)]" unoptimized />
            ...
            <button type="button" onClick={() => void openCase()} disabled={opening} className="mt-6 w-full rounded-2xl bg-[linear-gradient(90deg,#6730df,#9138f5,#ff7f2a)] py-4 font-black tracking-[.14em] transition hover:brightness-110 disabled:cursor-wait disabled:opacity-50">{opening ? "ОТКРЫВАЕМ..." : "ОТКРЫТЬ КЕЙС"}</button>
          </>
        ) : (
          <>
