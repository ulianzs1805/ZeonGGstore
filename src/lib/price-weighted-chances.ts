...
function readRarityWeights() {
  ...
  try { const parsed = JSON.parse(configured) as Record<string, unknown>; const weights = { ...DEFAULT_RARITY_WEIGHTS }; for (const [rarity, weight] of Object.entries(parsed)) if (typeof weight === "number" && Number.isFinite(weight) && weight > 0) weights[rarity.toUpperCase()] = weight; return weights; } catch { return DEFAULT_RARITY_WEIGHTS; }
}
...
export function isKnifeDrop(drop: Pick<ProbabilityDrop, "name">) {
  const name = (drop.name ?? "").trim().toLocaleLowerCase("ru-RU");
  return /butterfly|karambit|kunai|tanto|kukri|fang|m9 bayonet|bayonet|flip|gut|huntsman|falchion|stiletto|jkommando|scorpion|daggers|dual daggers|knife/i.test(name);
}
...
export function calculateFinalProbabilities<T extends ProbabilityDrop>(drops: T[], mode: ProbabilityMode = "DYNAMIC") {
  ...
  if (mode === "MANUAL") return drops.map(d => Number.isFinite(d.probability) && d.probability > 0 ? d.probability : 0);
