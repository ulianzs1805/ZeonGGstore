  ...
  useEffect(() => {
    const loadCases = async () => {
      try {
        ...
        let data: {
          cases?: Array<{ id: string; slug: string; name: string; price: number; image: string }>;
          error?: string;
        } = {};
        ...
        setCases(
          (data.cases ?? []).map((item, index) => ({
            ...item,
            ...caseVisuals[index % caseVisuals.length],
          })),
        );
      } catch (error) {
        setCasesError(
          error instanceof Error ? error.message : "Не удалось загрузить кейсы.",
        );
      }
    };
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#060812] text-white">
      ...
      <section className="px-3 pb-8 pt-5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-[1440px] overflow-hidden rounded-[30px] border border-violet-400/20 bg-[#090d18] shadow-[0_24px_100px_rgba(0,0,0,0.52),0_0_70px_rgba(124,58,237,0.10)]">
          ...
          <div className="relative isolate min-h-[700px] overflow-hidden bg-[radial-gradient(circle_at_12%_18%,rgba(124,58,237,0.20),transparent_30%),radial-gradient(circle_at_88%_18%,rgba(255,132,43,0.16),transparent_28%),radial-gradient(circle_at_82%_70%,rgba(139,92,246,0.18),transparent_34%),linear-gradient(90deg,#090d18_0%,#090d18_48%,rgba(9,13,24,0.45)_100%)] sm:min-h-[700px] lg:min-h-[690px]">
            ...
            <div className="relative z-10 flex min-h-[700px] flex-col justify-between px-5 py-10 sm:min-h-[700px] sm:px-8 sm:py-12 lg:min-h-[690px] lg:px-12 lg:py-14">
              <div className="max-w-2xl">
                <div className="mb-7 flex items-center gap-3">
                  <span className="h-2.5 w-2.5 rounded-full bg-violet-400 shadow-[0_0_20px_rgba(167,139,250,0.95)]" />
                  <span className="text-[0.68rem] font-black uppercase tracking-[0.34em] text-violet-200/90">
                    ZEONGG STORE · BETA
                  </span>
                </div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <Link
                    href="#cases"
                    className="inline-flex min-w-[185px] items-center justify-center rounded-2xl border border-white/10 bg-[#0a0f1b]/55 px-7 py-4 text-sm font-black uppercase tracking-[0.12em] text-slate-200 backdrop-blur-md transition hover:border-orange-300/35 hover:bg-orange-400/[0.06] hover:text-white"
                  >
                    Смотреть дроп
                  </Link>
                </div>
              ...
              <div className="mt-12 flex justify-center lg:justify-start">
                <div className="inline-flex items-center gap-3 rounded-full border border-white/[0.08] bg-[#0a0f1b]/80 px-5 py-3 backdrop-blur-xl">
                  <span className="h-2.5 w-2.5 rounded-full bg-gradient-to-r from-violet-400 to-orange-400 shadow-[0_0_16px_rgba(167,139,250,0.95)]" />
                  <span className="text-[0.64rem] font-black uppercase tracking-[0.22em] text-slate-200">
                    Дроп в реальном времени
                  </span>
                </div>
              </div>
            </div>
      ...
      <section id="cases" className="px-4 pb-12 pt-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-[1440px]">
          <div className="mb-8 flex items-end justify-between gap-4">
            <div>
              <p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-violet-300/75">Выбор игроков</p>
              <h2 className="mt-2 text-3xl font-black uppercase tracking-[-0.05em] text-white">Популярные кейсы</h2>
            </div>
            <Link href="/case" className="text-sm font-bold text-violet-200 transition hover:text-orange-200">Все кейсы →</Link>
          </div>

          {casesError && (
            <p className="mb-6 rounded-xl border border-red-300/20 bg-red-500/10 p-4 text-sm text-red-200">{casesError}</p>
          )}

          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {cases.map((item) => (
              <article key={item.id} className="group relative overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#0a0f18] p-4 transition duration-300 hover:-translate-y-1 hover:border-violet-300/25">
                <div className={`absolute inset-0 bg-gradient-to-br ${item.glow} opacity-70`} />
                <div className="relative">
                  <div className={`relative h-56 overflow-hidden rounded-[22px] border ${item.ring} bg-[#080c15]`}>
                    <Image src={item.image} alt={item.name} fill sizes="(max-width: 1280px) 50vw, 25vw" className="object-contain p-5 transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="flex items-center justify-between gap-3 px-1 pb-1 pt-4">
                    ...
                    <span className="shrink-0 text-sm font-black text-amber-200">{item.price} Z</span>
                  </div>
                  <Link href={`/case?caseId=${item.id}`} className="mt-4 flex items-center justify-center rounded-xl border border-violet-400/25 bg-violet-500/[0.08] py-3 text-xs font-black uppercase tracking-[0.14em] text-violet-100 transition hover:bg-violet-500/20">
                    Открыть
                  </Link>
                </div>
              </article>
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-[1440px] gap-3 md:grid-cols-5">
          {benefits.map((title, index) => (
            <div key={title} className="rounded-2xl border border-white/[0.08] bg-[#0a0f18] p-4">
              <div className="flex items-center gap-3">
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${index % 2 ? "bg-orange-400/10 text-orange-200" : "bg-violet-400/10 text-violet-200"}`}>✦</span>
