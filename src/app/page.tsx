  ...
  useEffect(() => {
    const loadCases = async () => {
      try {
        ...
        setCases((data.cases ?? []).map((item, index) => ({
          ...item,
      ...
      <section className="px-3 pb-8 pt-5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-[1440px] overflow-hidden rounded-[30px] border border-violet-400/20 bg-[#090d18] shadow-[0_24px_100px_rgba(0,0,0,0.52),0_0_70px_rgba(124,58,237,0.10)]">
          ...
          <div className="relative isolate min-h-[700px] overflow-hidden bg-[radial-gradient(circle_at_12%_18%,rgba(124,58,237,0.20),transparent_30%),radial-gradient(circle_at_88%_18%,rgba(255,132,43,0.16),transparent_28%),radial-gradient(circle_at_82%_70%,rgba(139,92,246,0.18),transparent_34%),linear-gradient(90deg,#090d18_0%,#090d18_48%,rgba(9,13,24,0.45)_100%)] sm:min-h-[650px] lg:min-h-[620px]">
            <div className="pointer-events-none absolute -right-[34%] top-[-7%] h-[118%] w-[118%] bg-[url('/zeongg-logo.webp')] bg-[length:115%_auto] bg-[position:100%_48%] bg-no-repeat opacity-100 mix-blend-screen sm:-right-[12%] sm:w-[82%] sm:bg-[length:145%_auto] sm:bg-[position:100%_47%] lg:-right-[6%] lg:w-[68%] lg:bg-[length:178%_auto] lg:bg-[position:100%_46%]" />
            ...
            <div className="relative z-10 flex min-h-[700px] flex-col justify-between px-5 py-10 sm:min-h-[650px] sm:px-8 sm:py-12 lg:min-h-[620px] lg:px-12 lg:py-14">
              <div className="max-w-2xl">
                <div className="mb-7 flex items-center gap-3">
                  ...
                  <span className="text-[0.68rem] font-black uppercase tracking-[0.34em] text-violet-200/90">ZEONGG STORE · BETA</span>
                </div>

                <h1 className="max-w-[820px] text-[2.9rem] font-black leading-[0.9] tracking-[-0.06em] text-white sm:text-6xl lg:text-[4.7rem]">
                  <span className="whitespace-nowrap bg-gradient-to-r from-violet-300 via-white to-orange-300 bg-clip-text text-transparent">ZeonGGstore—</span>
                  <br />
                  дроп всегда
                  <br />
                  с тобой.
              ...
              <div className="mt-8 flex justify-center lg:justify-start">
                <div className="inline-flex items-center gap-3 rounded-full border border-white/[0.08] bg-[#0a0f1b]/80 px-5 py-3 backdrop-blur-xl">
                  ...
                  <span className="text-[0.64rem] font-black uppercase tracking-[0.22em] text-slate-200">Дроп в реальном времени</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {cases.map((item) => (
              <article key={item.id} className="group relative overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#0a0f18] p-4 transition duration-300 hover:-translate-y-1 hover:border-violet-300/25">
                ...
                <div className="relative">
                  <div className={`relative h-56 overflow-hidden rounded-[22px] border ${item.ring} bg-[#080c15]`}><Image src={item.image} alt={item.name} fill sizes="(max-width: 1280px) 50vw, 25vw" className="object-contain p-5 transition duration-500 group-hover:scale-105" /></div>
                  <div className="flex items-center justify-between gap-3 px-1 pb-1 pt-4"><p className="truncate text-base font-black text-white">{item.name}</p><span className="shrink-0 text-sm font-black text-amber-200">{item.price} Z</span></div>
                  <Link href={`/case?caseId=${item.id}`} className="mt-4 flex items-center justify-center rounded-xl border border-violet-400/25 bg-violet-500/[0.08] py-3 text-xs font-black uppercase tracking-[0.14em] text-violet-100 transition hover:bg-violet-500/20">Открыть</Link>
                </div>
              </article>
            ))}
          </div>
