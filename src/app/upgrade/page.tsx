...
const chanceFor = (input: number, target: number) =>
  Math.max(MIN_CHANCE, Math.min(100, target > 0 ? (input / target) * 100 : MIN_CHANCE));
...
export default function UpgradePage() {
  ...
  const availableTargets = useMemo(() => {
    ...
    return targets.filter((x) => {
      if (total <= 0 || x.price <= total) return false;
      return !q || x.name.toLowerCase().includes(q) || x.rarity.toLowerCase().includes(q);
    });
  ...
  function autoTargetForChance(band: ChanceBand) {
    ...
    const selected = candidates.reduce((best, item) => {
      const distance = Math.abs(chanceFor(total, item.price) - midpoint);
      const bestDistance = Math.abs(chanceFor(total, best.price) - midpoint);
  ...
  function autoTargetForMultiplier(multiplier: number) {
    ...
    const selected = candidates.reduce((best, item) =>
      Math.abs(item.price - desired) < Math.abs(best.price - desired) ? item : best
    );
    setTargetId(selected.id);
    setResult(null);
    setError("");
  ...
  async function upgrade() {
    if ((!input && !noSkinMode) || !target || target.price <= total || topUp > balance || busy || spinning || total <= 0) return;
    ...
    try {
      const r = await fetch("/api/upgrader", {
        ...
        body: JSON.stringify({
          itemId: noSkinMode ? undefined : input?.id,
          targetId: target.id,
          balanceTopUp: attemptTopUp,
        ...
        {loading ? <div className="rounded-2xl border border-white/10 bg-white/[.02] p-10 text-center text-slate-400">Загружаем апгрейдер...</div> : <>
          <section className="rounded-3xl border border-violet-300/10 bg-[#0b0d15] p-3 sm:p-5">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <button type="button" onClick={toggleNoSkin} disabled={spinning || busy} className={`rounded-lg border px-3 py-2 text-[9px] font-black uppercase tracking-wider ${noSkinMode ? "border-orange-400/40 bg-orange-500/15 text-orange-300" : "border-white/10 bg-black/20 text-slate-400"}`}>Без скина → за баланс</button>
            ...
            <div className="grid grid-cols-[minmax(82px,150px)_minmax(130px,1fr)_minmax(100px,180px)] items-center gap-2 sm:gap-6">
              <SelectedCard item={noSkinMode ? null : input} label={noSkinMode ? "БЕЗ СКИНА" : "ТВОЙ СКИН"} compact balanceMode={noSkinMode} />
              ...
              <SelectedCard item={shownTarget} label="ЦЕЛЕВОЙ СКИН" target />
            </div>
            ...
            <div className="mt-3 rounded-xl border border-violet-300/10 bg-violet-500/[.04] p-3">
              ...
              <div className="mt-2 flex gap-2">{[25, 50, 100].filter((v) => v <= balance).map((v) => <button key={v} type="button" disabled={spinning || busy} onClick={() => addTopUp(v)} className="rounded bg-violet-500/15 px-2 py-1 text-[9px] font-bold text-violet-300">+{v} Z</button>)}</div>
            </div>
            ...
            <div className="mt-3 rounded-xl border border-white/10 bg-black/15 p-3">
              ...
              <div className="grid grid-cols-3 gap-2">{CHANCE_BANDS.map((band) => <button key={band.label} type="button" disabled={total <= 0 || spinning || busy} onClick={() => autoTargetForChance(band)} className="rounded-lg border border-violet-400/20 bg-violet-500/10 px-2 py-2 text-[9px] font-black text-violet-200 disabled:opacity-40">Шанс {band.label}</button>)}</div>
              <div className="mt-2 grid grid-cols-3 gap-2">{MULTIPLIERS.map((m) => <button key={m} type="button" disabled={total <= 0 || spinning || busy} onClick={() => autoTargetForMultiplier(m)} className="rounded-lg border border-orange-400/20 bg-orange-500/10 px-2 py-2 text-[9px] font-black text-orange-200 disabled:opacity-40">Цель ×{m}</button>)}</div>
            </div>
...
function SelectedCard({ item, label, compact, target, balanceMode }: { item: Item | null; label: string; compact?: boolean; target?: boolean; balanceMode?: boolean }) {
  return <div className="text-center"><p className="mb-1 text-[7px] font-black uppercase tracking-widest text-slate-500 sm:text-[9px]">{label}</p><div className={`relative mx-auto rounded-xl border ${item ? (target ? "border-orange-400/30 bg-orange-500/[.05]" : "border-violet-400/30 bg-violet-500/[.06]") : "border-dashed border-white/10 bg-black/20"} ${compact ? "h-24 w-full sm:h-32" : "h-28 w-full sm:h-40"}`}><span className="absolute inset-0 flex items-center justify-center text-[8px] text-slate-600">{!item && (balanceMode ? "Z-Coin" : "Выбери")}</span>{item && <Image src={item.image} alt={item.name} fill className="object-contain p-2" sizes="180px" unoptimized />}</div>{item && <><p className="mt-1 line-clamp-1 text-[9px] font-black sm:text-xs">{item.name}</p><p className="text-[9px] font-black text-yellow-300 sm:text-xs">{money(item.price)} Z</p></>}</div>;
}
