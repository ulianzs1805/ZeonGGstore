...
function makeParticles(seed: number) {
  ...
  for (let row = 0; row < 5; row++) {
    for (let col = 0; col < 9; col++) {
      const jitterX = (next(id * 11 + 1) - 0.5) * 7;
      const jitterY = (next(id * 11 + 2) - 0.5) * 8;
      particles.push({
        id,
        x: (next(id * 11 + 3) - 0.5) * (190 + next(id * 11 + 4) * 180),
        y: (next(id * 11 + 5) - 0.5) * (145 + next(id * 11 + 6) * 155),
        size: 17 + next(id * 11 + 7) * 16,
        rotate: (next(id * 11 + 8) - 0.5) * 620,
        delay: next(id * 11 + 9) * 230,
        sourceX: 12 + col * 9.5 + jitterX,
        sourceY: 22 + row * 14 + jitterY,
      });
      id++;
    }
  }
...
export default function UpgradePage() {
  ...
  async function playResult(data: Result) {
    ...
    if (data.success) window.setTimeout(() => setPhase("gather"), 2050);
    window.setTimeout(async () => {
      try {
        ...
        setTargetId(""); setTopUp(0); setAttempt(null); setParticles([]); setPhase("idle"); setAnimating(false);
      }
    }, BREAK_MS);
  }
  ...
  async function upgrade() {
    ...
    try {
      ...
      setAttempt(null); setError(e instanceof Error ? e.message : "Ошибка апгрейда");
    ...
    <div className="mx-auto max-w-[1280px] overflow-hidden">
      <section className="relative overflow-hidden border-y border-violet-400/10 bg-[radial-gradient(circle_at_50%_34%,rgba(110,49,255,.20),transparent_30%),radial-gradient(circle_at_20%_25%,rgba(255,119,34,.09),transparent_28%),linear-gradient(180deg,#0d1020_0%,#090b16_82%)] px-4 py-8 sm:px-8 sm:py-10">
        ...
        <div className="relative grid grid-cols-[.9fr_1.25fr_.9fr] items-center gap-2 sm:gap-8">
          <WeaponSlot item={displayInput} side="left" onShuffle={() => setInputId("")} hidden={animating && phase !== "gather"} />
        ...
        {animating && attempt && result && particles.length > 0 && <ParticleAnimation success={result.success} target={attempt.target} input={attempt.input} particles={particles} phase={phase} />}
...
function WeaponSlot({ item, side, onShuffle, hidden }: { item: Item | null; side: "left" | "right"; onShuffle: () => void; hidden?: boolean }) {
  return <div className={`relative z-10 flex flex-col items-center justify-center gap-3 text-center transition-all duration-500 ${hidden ? "scale-95 opacity-0" : "scale-100 opacity-100"}`}>
    ...
    <div className="relative h-20 w-full max-w-[180px] rounded-2xl border border-violet-400/15 bg-[#111424] p-3 shadow-[0_0_30px_rgba(95,48,255,.10)] sm:h-32 sm:max-w-[250px]">{item ? <Image src={item.image} alt={item.name} fill className="object-contain p-3 drop-shadow-[0_0_20px_rgba(116,65,255,.45)]" unoptimized /> : <div className="grid h-full place-items-center text-[9px] font-black uppercase tracking-[.14em] text-zinc-600">Выбери предмет</div>}</div>
...
function ParticleAnimation({ success, target, input, particles, phase }: { success: boolean; target: Item; input: Item | null; particles: Particle[]; phase: Phase }) {
  const source = (item: Item | null, side: "left" | "right", gather = false) => {
    ...
    return <div className={`pointer-events-none absolute z-40 h-[108px] w-[180px] sm:h-[150px] sm:w-[250px] ${side === "left" ? "left-[1%] sm:left-[10%]" : "right-[1%] sm:right-[10%]"} top-[74px] sm:top-[112px]`}>
      {particles.map((p) => <span key={`${side}-${gather ? "g" : "b"}-${p.id}`} className={`upgrade-fragment ${gather ? "upgrade-fragment-gather" : "upgrade-fragment-burst"}`} style={{
        width: `${p.size}px`, height: `${Math.max(13, p.size * 0.72)}px`,
        left: `${p.sourceX}%`, top: `${p.sourceY}%`,
        backgroundImage: `url(${JSON.stringify(item.image)})`,
        backgroundSize: "250px 150px", backgroundRepeat: "no-repeat",
        backgroundPosition: `${-p.sourceX * 2.5}px ${-p.sourceY * 1.5}px`,
        ["--x" as string]: `${gather ? -p.x : p.x}px`, ["--y" as string]: `${gather ? -p.y : p.y}px`, ["--r" as string]: `${p.rotate}deg`,
        animationDelay: `${p.delay}ms`,
      } as React.CSSProperties} />)}
    </div>;
  };
  return <div className="pointer-events-none absolute inset-0 z-30 overflow-visible">
    {success ? <>{source(target, "right")}{phase === "gather" && source(target, "left", true)}</> : <>{source(input, "left")}{source(target, "right")}</>}
    <style jsx>{`
      .upgrade-fragment{position:absolute;display:block;border-radius:4px;box-shadow:0 0 18px rgba(122,70,255,.34);will-change:transform,opacity,filter;opacity:0;}
      .upgrade-fragment-burst{animation:upgradeBurst 3.05s cubic-bezier(.12,.72,.16,1) forwards;}
      .upgrade-fragment-gather{animation:upgradeGather 2.5s cubic-bezier(.16,.78,.18,1) forwards;}
      @keyframes upgradeBurst{0%{opacity:0;transform:translate(-50%,-50%) translate(0,0) rotate(0deg) scale(.95);filter:brightness(1.45)}7%{opacity:1}48%{opacity:1;filter:brightness(1.1)}100%{opacity:0;transform:translate(-50%,-50%) translate(var(--x),var(--y)) rotate(var(--r)) scale(.48);filter:brightness(.65)}}
      @keyframes upgradeGather{0%{opacity:0;transform:translate(-50%,-50%) translate(var(--x),var(--y)) rotate(var(--r)) scale(.42);filter:brightness(.7)}12%{opacity:1}68%{opacity:1;filter:brightness(1.22)}100%{opacity:0;transform:translate(-50%,-50%) translate(0,0) rotate(0deg) scale(1);filter:brightness(1.75)}}
    `}</style>
  </div>;
}
