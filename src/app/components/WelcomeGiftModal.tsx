...
type WelcomeResponse = { case?: WelcomeCase | null; show?: boolean };
...
export default function WelcomeGiftModal() {
  ...
  useEffect(() => {
    // Важно: сначала полностью определяем текущий аккаунт.
    // Гостям и уже зарегистрированным ранее игрокам окно не показываем.
    ...
    fetch("/api/welcome-case", { cache: "no-store", credentials: "include" })
      ...
      .then((data) => {
        ...
        // Решение принимает сервер по userId аккаунта, а не ссылка,
        // браузер, localStorage или sessionStorage.
        if (data?.show && data.case) {
          setGiftCase(data.case);
          setOpen(true);
        } else {
          setGiftCase(null);
          setOpen(false);
  ...
  if (status !== "authenticated" || loading || !open || !giftCase) return null;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02040a]/80 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-lg overflow-hidden rounded-[32px] border border-violet-300/30 bg-[#080d18] p-6 shadow-[0_0_90px_rgba(124,58,237,0.4)] sm:p-8">
        ...
        <div className="relative text-center">
          ...
          <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-slate-300">Для новых игроков мы подготовили один случайный приветственный кейс.</p>
          ...
          <div className="mt-7 rounded-[24px] border border-violet-300/25 bg-[#0d1320] p-4">
            ...
            <p className="mt-1 text-xs text-violet-200">Этот подарок закреплён именно за твоим аккаунтом и больше повторно не выдаётся.</p>
