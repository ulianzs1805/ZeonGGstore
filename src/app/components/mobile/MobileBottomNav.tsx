...
function IconZ() { return <span className="text-xl font-black italic leading-none">Z</span>; }
...
export default function MobileBottomNav() {
  ...
  const active = (item: NavItem) => {
    ...
    if (item.label === "Пополнить") return false;
  ...
  return <>
    ...
    <nav className="fixed inset-x-2 bottom-[max(0.55rem,env(safe-area-inset-bottom))] z-[80] mx-auto flex h-[70px] max-w-[720px] items-center rounded-[26px] border border-white/10 bg-[#11131c]/95 p-1 shadow-[0_-8px_40px_rgba(0,0,0,0.38)] backdrop-blur-2xl md:hidden" aria-label="Мобильная навигация">
      {items.slice(0, 2).map((item) => <Link key={item.label} href={item.href!} className={itemClass(active(item))}>{item.icon}<span>{item.label}</span></Link>)}
      <Link href="/account" className="relative -mt-7 flex min-w-[68px] flex-1 flex-col items-center gap-1 text-[9px] font-black uppercase tracking-[0.06em] text-violet-100">
        <span className="flex h-14 min-w-14 items-center justify-center gap-1 rounded-[22px] border border-violet-200/30 bg-gradient-to-b from-violet-300 via-fuchsia-400 to-violet-600 px-3 text-white shadow-[0_0_28px_rgba(139,92,246,0.32)]"><IconZ />{balance !== null && <span className="max-w-[48px] truncate text-xs font-black not-italic">{new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 0 }).format(balance)}</span>}</span>
        <span>Z-Coin</span>
      </Link>
      {items.slice(2).map((item) => <Link key={item.label} href={item.href!} className={itemClass(active(item))}>{item.icon}<span>{item.label}</span></Link>)}
    </nav>
  </>;
}
