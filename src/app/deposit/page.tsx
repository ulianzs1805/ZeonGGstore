...
type PaymentMethod = "sbp" | "bank_card" | "tinkoff_bank" | "sberbank";
...
function DepositContent() {
  ...
  return <main className="min-h-screen bg-[#05070d] text-white"><Header/><section className="px-3 py-5 pb-28 sm:px-6 sm:py-10 lg:px-8"><div className="mx-auto max-w-2xl rounded-[28px] border border-violet-400/20 bg-[#0a0f18]/95 p-4 shadow-[0_0_70px_rgba(124,58,237,.12)] sm:rounded-[30px] sm:p-8">
    ...
    {!paymentId && <>
      <div className="mt-7"><div className="mb-2 flex items-center justify-between"><label className="text-xs font-black uppercase tracking-wide text-slate-400">Способ оплаты</label><span className="text-[10px] font-bold text-slate-600">ЮKassa</span></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{paymentMethods.map((item) => <button key={item.id} type="button" onClick={() => setMethod(item.id)} className={`rounded-2xl border p-3 text-left transition active:scale-[.98] ${method === item.id ? "border-violet-300/50 bg-violet-400/10 shadow-[0_0_22px_rgba(168,85,247,.12)]" : "border-white/10 bg-white/[.025] hover:border-white/20"}`}><span className={`grid h-9 w-9 place-items-center rounded-xl text-sm font-black ${method === item.id ? "bg-violet-400/20 text-violet-100" : "bg-white/5 text-slate-300"}`}>{item.icon}</span><span className="mt-2 block text-[11px] font-black text-white">{item.title}</span><span className="mt-0.5 block text-[9px] leading-3 text-slate-500">{item.subtitle}</span></button>)}</div></div>
    ...
    {statusCard}
    <div className="mt-5 rounded-2xl border border-white/8 bg-white/[.02] p-4 text-[10px] leading-5 text-slate-500"><b className="text-slate-300">Важно:</b> баланс меняется только после подтверждённого платежа от ЮKassa. Историю пополнений можно будет смотреть в разделе операций.</div>
  </div></section></main>;
}
