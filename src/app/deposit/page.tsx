...
function DepositContent() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/bonuses/fortune", { cache: "no-store" }).then(async (r) => {
      ...
      if (!r.ok) return;
      const best = data.bonusInventory?.[0];
      if (!fromWheel && best?.label) {
        try {
          const parsed = JSON.parse(best.label);
          if (parsed.code) setBonus({ code: parsed.code, percent: parsed.percent });
        } catch {}
      }
    }).catch(() => undefined);
  }, [params]);
...
export default function DepositPage() {
  return <Suspense fallback={<main className="min-h-screen bg-[#05070d] text-white"><Header/><div className="mx-auto max-w-2xl px-4 py-12 text-center text-slate-400">Загрузка пополнения…</div></main>}><DepositContent /></Suspense>;
}
