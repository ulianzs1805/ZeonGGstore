...
function DepositContent() {
  ...
  const [method] = useState<PaymentMethod>("yoomoney");
  ...
  const createPayment = async () => {
    setError("");
    if (value < 1 || value > 40000) { setError("Сумма пополнения — от 1 ₽ до 40 000 ₽."); return; }
    setLoading(true);
    try {
  ...
  return <main className="min-h-screen bg-[#05070d] text-white"><Header/><section className="px-3 py-5 pb-28 sm:px-6 sm:py-10 lg:px-8"><div className="mx-auto max-w-2xl rounded-[28px] border border-violet-400/20 bg-[#0a0f18]/95 p-4 shadow-[0_0_70px_rgba(124,58,237,.12)] sm:rounded-[30px] sm:p-8">
    ...
    {!paymentId && <>
      ...
      <p className="mt-3 rounded-xl border border-violet-300/15 bg-violet-400/5 px-4 py-3 text-[10px] leading-4 text-slate-400">ЮMoney откроет отдельную страницу подтверждения оплаты. Зачисление произойдёт только после серверного HTTP-уведомления с проверенной подписью.</p>
