...
export default function BestDrop({ bestDrop }: Props) {
  ...
  const imageSrc = resolveSkinImage(bestDrop.name, bestDrop.image);
  return (
    <div className="mt-6 rounded-[26px] border border-yellow-400/40 bg-zinc-950 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-yellow-300/60">
      ...
      <div className="mt-3 flex items-center gap-4">
        <div className="relative h-20 w-20 shrink-0"><Image src={imageSrc} alt={bestDrop.name} fill className="object-contain" sizes="80px" unoptimized priority /></div>
