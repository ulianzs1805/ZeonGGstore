...
export default function AdminPanel({ role, email, staffId }: { role: Role; email: string; staffId: string | null }) {
  ...
  const toggleCase = async (item: CatalogCase) => {
    ...
    else setCases((current) => current.map((candidate) => candidate.id === item.id ? { ...candidate, isActive: !candidate.isActive } : candidate));
  };
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#06040d] px-4 py-5 text-white sm:px-8 sm:py-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-5 lg:grid-cols-[0.7fr_1.3fr]">
      <div className={`fixed inset-y-0 left-0 z-50 w-[min(84vw,320px)] transform border-r border-violet-300/20 bg-[#0b0715] p-4 shadow-2xl transition-transform lg:static lg:z-auto lg:w-auto lg:translate-x-0 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none ${mobileNavOpen ? "translate-x-0" : "-translate-x-full"}`}><div className="mb-4 flex items-center justify-between lg:hidden"><span className="font-black text-violet-100">Навигация</span><button type="button" onClick={() => setMobileNavOpen(false)} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-300" aria-label="Закрыть навигацию">×</button></div><nav className="space-y-2">{sections.filter((item) => item.visible).map((item) => <button key={`${item.id}-${item.label}`} type="button" onClick={() => setSection(item.id)} className={`w-full rounded-2xl border p-3 text-left text-sm font-semibold transition sm:p-4 ${section === item.id ? "border-violet-300/50 bg-violet-400/10 text-white shadow-[0_0_24px_rgba(124,58,237,0.2)]" : "border-violet-300/15 bg-white/[0.04] text-slate-300 hover:border-violet-300/30"}`}>{item.label}</button>)}</nav></div>
      <section className="rounded-3xl border border-violet-300/20 bg-white/[0.045] p-5 shadow-[0_0_45px_rgba(124,58,237,0.12)] sm:p-7">
        ...
        {section === "skinPrices" && (role === "DEV" || role === "NPN1_DEV") && <><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.24em] text-violet-300">ECONOMY CONTROL</p><h2 className="mt-2 text-2xl font-black">Стоимость скинов</h2><p className="mt-2 text-sm text-slate-400">Изменение цен с ограничениями роли и серверной проверкой.</p></div><SkinPricePanel role={role} /></>}
        {section === "create" && (createdCase ? <div className="py-10 text-center"><p className="text-4xl text-emerald-300">✓</p><h2 className="mt-4 text-2xl font-black">Кейс успешно создан</h2><p className="mt-2 text-slate-300">{createdCase.name} · {createdCase.drops} предмета</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href={`/case?caseId=${encodeURIComponent(createdCase.slug)}`} className="rounded-xl bg-violet-500 px-5 py-3 text-sm font-black hover:bg-violet-400">Открыть кейс</Link><button type="button" onClick={() => { setCreatedCase(null); setStep(1); setName(""); setCaseImage(""); setDrops([newDrop()]); }} className="rounded-xl border border-white/15 px-5 py-3 text-sm font-bold text-slate-200">Создать ещё</button></div></div> : <>
          <div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.24em] text-violet-300">Шаг {step} / 2</p><h2 className="mt-2 text-2xl font-black">{step === 1 ? "Создание кейса" : "Добавьте предметы"}</h2><p className="mt-2 text-sm text-slate-400">Сервер повторно проверит все данные перед transaction.</p></div>
          {step === 1 ? <div className="space-y-5"><ImageUploadField label="Изображение кейса" value={caseImage} onChange={setCaseImage} /><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Название кейса" className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm outline-none focus:border-violet-300/60" />{canEditEconomy && <input type="number" min="1" value={casePrice} onChange={(event) => setCasePrice(Number(event.target.value))} placeholder="Цена кейса в Z-Coin" className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm outline-none focus:border-violet-300/60" />}{!canEditEconomy && <p className="rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-xs text-slate-400">Цена кейса определяется системной policy для ADMIN.</p>}<button type="button" onClick={next} className="w-full rounded-xl bg-violet-500 px-4 py-3 text-sm font-black hover:bg-violet-400">Далее</button></div> : <div><div className="mb-4 flex items-center justify-between"><span className={`text-sm font-bold ${Math.abs(totalProbability - 100) < 0.001 || role === "ADMIN" ? "text-emerald-300" : "text-amber-300"}`}>{role === "ADMIN" ? "Вероятности распределит система" : `Сумма вероятностей: ${totalProbability}%`}</span><button type="button" onClick={() => setDrops((current) => [...current, newDrop()])} className="rounded-lg border border-violet-300/30 px-3 py-2 text-xs font-bold text-violet-100">+ Добавить предмет</button></div><div className="space-y-4">{drops.map((drop, index) => <article key={index} className="rounded-2xl border border-white/10 bg-black/15 p-4"><div className="mb-3 flex items-center justify-between"><h3 className="font-bold">Drop #{index + 1}</h3>{drops.length > 1 && <button type="button" onClick={() => setDrops((current) => current.filter((_, dropIndex) => dropIndex !== index))} className="text-xs font-bold text-red-300">Удалить</button>}</div><ImageUploadField label="Изображение предмета" value={drop.image} onChange={(value) => updateDrop(index, { image: value })} /><div className="mt-3 grid gap-3 sm:grid-cols-2"><input value={drop.name} onChange={(event) => updateDrop(index, { name: event.target.value })} placeholder="Название" className="rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm" /><select value={drop.rarity} onChange={(event) => updateDrop(index, { rarity: event.target.value as Rarity })} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm">{rarities.map((rarity) => <option key={rarity}>{rarity}</option>)}</select><input type="number" min="1" value={drop.price} onChange={(event) => updateDrop(index, { price: Number(event.target.value) })} placeholder="Цена Z-Coin" className="rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm" />{canEditEconomy && <input type="number" min="0.01" max="100" step="0.01" value={drop.probability} onChange={(event) => updateDrop(index, { probability: Number(event.target.value) })} placeholder="Вероятность %" className="rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm" />}</div></article>)}</div>{message && <p className="mt-4 rounded-xl border border-violet-300/20 bg-violet-400/10 p-3 text-sm text-violet-100">{message}</p>}<div className="mt-6 flex gap-3"><button type="button" onClick={() => setStep(1)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-slate-300">Назад</button><button type="button" onClick={() => void createCase()} disabled={submitting || !validDraft} className="flex-1 rounded-xl bg-violet-500 px-4 py-3 text-sm font-black disabled:cursor-not-allowed disabled:opacity-40">{submitting ? "Проверяем..." : "Создать кейс"}</button></div></div>}
        </>)}
      </section>
    </div>
  </div></main>;
