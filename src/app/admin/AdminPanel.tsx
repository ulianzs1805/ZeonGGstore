...
export default function AdminPanel({ role, email, staffId }: { role: Role; email: string; staffId: string | null }) {
  ...
  const isDev = role === "DEV" || role === "NPN1_DEV";
  const isNpn = role === "NPN1_DEV";
  const tabs: Array<{ id: Section; label: string; visible: boolean }> = [
    ...
    { id: "roles", label: "Роли", visible: isDev },
    { id: "support", label: "Поддержка", visible: true },
    { id: "zcoin", label: "Z-Coin", visible: isDev },
  ...
  const renderSection = () => {
    ...
    if (section === "roles") return isDev ? <RoleManagementPanel role={role} /> : null;
    if (section === "support") return <AdminSupportPanel />;
