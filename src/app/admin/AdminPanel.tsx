...
export default function AdminPanel({ role, email, staffId }: { role: Role; email: string; staffId: string | null }) {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#06040d] px-4 py-5 text-white sm:px-8 sm:py-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="mb-5 lg:hidden"><label className="sr-only" htmlFor="admin-section-mobile">Раздел панели</label><select id="admin-section-mobile" value={section} onChange={(event) => setSection(event.target.value as Section)} className="w-full rounded-xl border border-violet-300/25 bg-[#0b0715] px-4 py-3 text-sm font-bold text-white outline-none focus:border-violet-300/60">{sections.filter((item) => item.visible).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
