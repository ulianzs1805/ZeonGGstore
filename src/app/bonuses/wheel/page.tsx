...
const bonusImages: Record<string, string> = {
  FREE_CASE: "/bonuses/IMG_9358.jpeg",
  ZCOIN_RAIN: "/bonuses/IMG_9359.jpeg",
  Z_BOOST: "/bonuses/IMG_9360.jpeg",
  LUCKY_DROP: "/bonuses/IMG_9361.jpeg",
  SAFE_OPEN: "/bonuses/IMG_9363.jpeg",
  DEPOSIT_BONUS: "/bonuses/IMG_9364.jpeg",
  DOUBLE_DROP: "/bonuses/IMG_9365.jpeg",
};
...
const descriptions: Record<string, string> = {
  ZEON_SECRET: "Случайная недостающая буква ZEONGG.",
  DEPOSIT_BONUS: "В центре выбирается бонус от 5% до 35%.",
  FREE_CASE: "Бесплатное открытие активного кейса.",
  ZCOIN_RAIN: "В центре выбирается пачка Z-Coin.",
  Z_BOOST: "Следующая подходящая награда получает +25% Z-Coin.",
  LUCKY_DROP: "Бонус для следующего открытия.",
  SAFE_OPEN: "Защита следующего открытия от слабого варианта.",
  DOUBLE_DROP: "Следующее открытие может дать дополнительный дроп.",
};
...
function Letter({ id, collected }: { id: string; collected: boolean }) {
  return (
    <div className={`relative grid h-11 w-11 place-items-center overflow-hidden rounded-xl border ${collected ? "border-orange-300/60 bg-orange-400/10" : "border-white/10 bg-black/25"}`}>
...
function InnerCell({ item }: { item: InnerItem }) {
  if (item.image) {
    return <TransparentImage src={item.image} alt={item.title} className="h-14 w-14 object-contain drop-shadow-[0_4px_10px_rgba(0,0,0,.8)]" />;
  }
  return (
    <div className="grid h-14 w-14 place-items-center rounded-xl border border-orange-300/20 bg-orange-400/5">
      <span className="text-xl font-black text-orange-200">{item.icon ?? item.title}</span>
    </div>
  );
}
...
function InnerRoulette({ data, spinning }: { data: InnerRouletteData; spinning: boolean }) {
  ...
  return (
    <div className="absolute left-[4%] right-[4%] top-1/2 z-[60] -translate-y-1/2 rounded-[24px] border border-orange-300/25 bg-[#070910] p-3 shadow-[0_30px_100px_rgba(0,0,0,.98)] sm:left-[7%] sm:right-[7%] sm:p-4">
      <div className="mb-2 flex items-center justify-between px-1 text-[8px] font-black uppercase tracking-[.2em]">
        <span className="text-slate-500">Финальный дроп</span>
        <span className="text-orange-300">{data.title}</span>
      </div>
      <div className="relative overflow-hidden rounded-xl border border-white/10 bg-[#020409] py-3">
        ...
        <div
          ...
          style={{
            transform: `translateX(calc(50% - ${card / 2}px - ${offset * (card + 8)}px))`,
            transition: spinning ? "transform 4.25s cubic-bezier(.08,.72,.12,1)" : "transform .3s ease-out",
          }}
        >
          {items.map((item, index) => (
            <div key={`${item.key}-${index}`} className="flex h-[104px] w-[116px] shrink-0 flex-col items-center justify-center rounded-xl border border-white/10 bg-[#11151d] px-2 text-center">
              <InnerCell item={item} />
              <b className="mt-1 line-clamp-2 text-[9px] leading-3 text-white">{item.title}</b>
              {item.subtitle && <small className="mt-0.5 text-[7px] text-slate-500">{item.subtitle}</small>}
            </div>
          ))}
        </div>
      </div>
...
function DrumCell({ item, index, angle, selected, letter }: { item: WheelItem; index: number; angle: number; selected: boolean; letter?: string }) {
  ...
  const image = isLetter ? letterImages[letter ?? "Z"] : bonusImages[item.type];
  const mid = index * angle + angle / 2;

  return (
    <div className="absolute left-1/2 top-1/2 h-[25%] w-[25%] -translate-x-1/2 -translate-y-1/2" style={{ transform: `rotate(${mid}deg) translateY(-151%)` }}>
      <div className={`relative h-full w-full rounded-full border-[6px] bg-[radial-gradient(circle_at_35%_30%,#202632,#090b10_62%,#050608)] shadow-[inset_0_0_22px_rgba(0,0,0,.95),0_8px_25px_rgba(0,0,0,.65)] sm:border-[8px] ${selected ? "border-orange-300 shadow-[0_0_38px_rgba(251,146,60,.75),inset_0_0_30px_rgba(251,146,60,.14)]" : "border-[#292e39]"}`}>
        ...
        <div className="absolute inset-[17%] grid place-items-center overflow-hidden rounded-full bg-[#07090d] shadow-[inset_0_0_22px_rgba(0,0,0,.9)]">
          {image && <TransparentImage src={image} alt={isLetter ? letter ?? "ZEONGG" : item.label} className="h-full w-full rounded-full object-contain drop-shadow-[0_0_12px_rgba(255,160,70,.55)]" />}
        </div>
...
export default function FortuneWheelPage() {
  ...
  useEffect(() => {
    fetch("/api/bonuses/fortune", { cache: "no-store" })
      .then(async (response) => {
  ...
  const background = useMemo(
    () => `conic-gradient(from -22.5deg, ${wheel.map((_, i) => `${i % 2 ? "#11151d" : "#171b24"} ${i * angle}deg ${(i + 1) * angle}deg`).join(",")})`,
    [wheel, angle],
  );
  ...
  async function spin() {
    ...
    try {
      ...
      const outer = window.setTimeout(() => {
        setSpinning(false);
        setResult(data);
        setRollingLetter(data.metadata?.letter);
        if (data.letterState) setLetterState(data.letterState);
        window.dispatchEvent(new Event("zeon-profile-updated"));

        if (data.innerRoulette && innerTypes.includes(data.rewardType)) {
          setInnerSpinning(true);
          const inner = window.setTimeout(() => setInnerSpinning(false), 4400);
          timers.current.push(inner);
        }
      }, 7250);
      timers.current.push(outer);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Ошибка барабана");
      setSpinning(false);
      setRollingLetter(undefined);
    }
  }
      ...
      <section className="px-3 pb-24 pt-3 sm:px-5 sm:pt-5">
        <div className="mx-auto max-w-[1500px] overflow-hidden rounded-[30px] border border-white/10 bg-[#080a0f] shadow-[0_35px_130px_rgba(0,0,0,.7)]">
          ...
          <div className="border-b border-white/10 bg-[radial-gradient(circle_at_50%_-20%,rgba(124,58,237,.24),transparent_60%)] px-4 py-8 text-center sm:px-8 sm:py-9">
            ...
            <p className="mx-auto mt-2 max-w-2xl text-xs leading-5 text-slate-500 sm:text-sm">Один барабан. Сервер выбирает бонус, а затем при необходимости запускает финальную рулетку.</p>
          </div>
          ...
          <div className="grid gap-6 p-3 sm:p-6 lg:grid-cols-[minmax(0,1fr)_330px] lg:p-8">
            <div className="rounded-[32px] border border-white/10 bg-[#090b10] p-3 shadow-[0_25px_90px_rgba(0,0,0,.6)] sm:p-6">
              <div className="mb-4 flex items-center justify-between px-1">
                <div><div className="text-[8px] font-black uppercase tracking-[.25em] text-slate-600">Главный призовой механизм</div><div className="mt-1 text-sm font-black">Крути и забирай бонус</div></div>
              ...
              <div className="relative mx-auto aspect-square w-full max-w-[680px] overflow-hidden rounded-full border border-white/10 bg-[#050609] p-3 shadow-[0_0_80px_rgba(0,0,0,.75)] sm:p-5">
                ...
                <div className="absolute inset-[6%] transition-transform duration-[7.2s] ease-[cubic-bezier(.22,.61,.36,1)]" style={{ transform: `rotate(${rotation}deg)` }}>
            ...
            <aside className="rounded-[32px] border border-white/10 bg-[#090b10] p-5">
              ...
              <div className="mt-4 space-y-2">
                {wheel.map((item) => (
                  <div key={item.type} className="flex items-center gap-3 rounded-2xl border border-white/5 bg-white/[.02] p-3">
                    <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-orange-300/20 bg-[#07090d]"><TransparentImage src={item.type === "ZEON_SECRET" ? letterImages.Z : bonusImages[item.type]} alt={item.label} className="h-full w-full object-contain" /></div>
                    <div className="min-w-0"><div className="truncate text-[10px] font-black uppercase text-white">{item.label}</div><div className="mt-1 text-[9px] leading-4 text-slate-500">{descriptions[item.type] || "Бонус барабана."}</div></div>
                  </div>
                ))}
              </div>
