...
function processedImage(value: string) {
  return value.startsWith("/uploads/processed/") && value.endsWith(".png") && value.length <= 240;
}

function makeSlug(value: string) {
  return value.normalize("NFKD").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `case-${Date.now()}`;
}
...
export async function POST(request: Request) {
  ...
  if (!validName(name)) return NextResponse.json({ error: "Невозможно создать кейс. Название кейса не прошло проверку." }, { status: 400 });
  if (!processedImage(image)) return NextResponse.json({ error: "Загрузите обработанное изображение через picker." }, { status: 400 });
  ...
  if (existing) return NextResponse.json({ error: "Кейс с таким названием или slug уже существует." }, { status: 409 });
  ...
  const chances = isAdmin ? normalizedDrops.map(() => 100 / normalizedDrops.length) : normalizedDrops.map((drop) => drop.chance);
  if (!validateChances(chances)) {
    return NextResponse.json({ error: "Невозможно сохранить кейс. Сумма вероятностей должна быть равна 100%." }, { status: 400 });
  }
    ...
    const created = await prisma.$transaction(async (transaction) => {
      const createdCase = await transaction.case.create({
        data: {
          ...
          drops: { create: normalizedDrops.map((drop, index) => ({ name: drop.name, rarity: drop.rarity, image: drop.image, price: drop.price, probability: chances[index] })) },
        },
        include: { drops: true },
      });
      await transaction.auditLog.create({
        data: {
          ...
          metadata: JSON.stringify({ name, drops: createdCase.drops.length }),
          status: "SUCCESS",
        },
      });
      return createdCase;
