...
export async function POST(request: Request) {
  ...
  const normalizedDrops = drops.map((drop) => ({ name: typeof drop.name === "string" ? drop.name.trim() : "", rarity: typeof drop.rarity === "string" ? drop.rarity.trim() : "", image: typeof drop.image === "string" ? drop.image.trim() : "", price: typeof drop.price === "number" ? drop.price : NaN, chance: typeof drop.probability === "number" ? drop.probability : typeof drop.chance === "number" ? drop.chance : NaN }));
  if (normalizedDrops.some((drop) => !validName(drop.name) || !validateRarity(drop.rarity) || !processedImage(drop.image) || !validateDropPrice(drop.price))) return NextResponse.json({ error: "Данные дропа не прошли проверку." }, { status: 400 });
  ...
  if (!validateChances(chances)) return NextResponse.json({ error: "Невозможно сохранить кейс. Сумма вероятностей должна быть равна 100%." }, { status: 400 });
  try {
