...
const LIMITS = {
  ADMIN: { total: 10, CASE: 4, ZCOIN: 6, maxZ: 3000 },
  DEV: { total: 5, CASE: 2, ZCOIN: 3, maxZ: 1500 },
} as const;
...
export async function GET() {
  ...
  const usage = { total: created.length, CASE: created.filter((item) => item.type === "CASE").length, ZCOIN: created.filter((item) => item.type === "ZCOIN").length };
...
export async function POST(request: Request) {
  ...
  const type = body?.type === "CASE" || body?.type === "ZCOIN" ? body.type : null;
  ...
  let caseId: string | null = null; let zCoinAmount: number | null = null;
  if (type === "CASE") {
    caseId = typeof body?.caseId === "string" ? body.caseId : null;
    if (!caseId || !await prisma.case.findFirst({ where: { id: caseId, isActive: true }, select: { id: true } })) return NextResponse.json({ error: "Выбери существующий активный кейс." }, { status: 400 });
  } else {
    zCoinAmount = Number(body?.zCoinAmount);
    if (!Number.isInteger(zCoinAmount) || zCoinAmount < 1) return NextResponse.json({ error: "Укажи положительное количество Z-Coin." }, { status: 400 });
  }
  ...
  if (user.role !== "NPN1_DEV") {
    ...
    const since = new Date(Date.now() - WINDOW_MS);
    const created = await prisma.promoCode.findMany({ where: { createdById: user.id, createdAt: { gte: since } }, select: { type: true } });
    const typeCount = created.filter((item) => item.type === type).length;
  ...
  try {
    const promo = await prisma.promoCode.create({ data: { code, type, zCoinAmount, caseId, maxActivations, expiresAt, createdById: user.id } });
