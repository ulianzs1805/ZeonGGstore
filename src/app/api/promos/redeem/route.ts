...
export async function POST(request: Request) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Atomic guard: two simultaneous requests cannot exceed maxActivations.
    ...
    if (promo.type === "ZCOIN") {
      ...
      await tx.transaction.create({
        data: { userId: user.id, type: "PROMO_ZCOIN", zCoinAmount: amount, status: "SUCCESS" },
      });
    ...
    await tx.freeCaseGrant.create({
      data: { userId: user.id, caseId: promo.caseId, sourcePromoId: promo.id },
    });
