...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as {
    itemId?: unknown; targetId?: unknown; idempotencyKey?: unknown; balanceTopUp?: unknown;
  } | null;
  ...
  const balanceTopUp = typeof body?.balanceTopUp === "number" && Number.isFinite(body.balanceTopUp)
    ? Math.floor(body.balanceTopUp) : 0;
  ...
  if (previous) {
    ...
    return NextResponse.json({
      ok: previous.status === "SUCCESS", replay: true, status: previous.status,
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!item) throw new Error("ITEMS_NOT_AVAILABLE");
      if (!target) throw new Error("TARGET_NOT_FOUND");
      if (!freshUser) throw new Error("USER_NOT_FOUND");

      // Апгрейдер всегда только улучшает предмет: цель обязана быть дороже исходного скина.
      if (target.price <= item.price) throw new Error("TARGET_MUST_BE_MORE_EXPENSIVE");
      if (balanceTopUp > freshUser.balance) throw new Error("INSUFFICIENT_BALANCE");

      const totalInputValue = item.price + balanceTopUp;
      ...
      if (!success) {
        return { success, chance, roll, target: publicItem(target), resultItem: null,
          inputItem: publicItem(item), inputValue: item.price, balanceTopUp, totalInputValue };
      }

      const resultItem = await tx.inventoryItem.create({
        data: { userId: user.id, itemId: target.id, name: target.name, rarity: target.rarity,
          image: target.image, price: target.price },
      });
      await tx.operation.create({
        data: { userId: user.id, type: "UPGRADE_REWARD", itemId: resultItem.id,
          label: `Получен ${target.name}`, amount: target.price, status: "SUCCESS",
