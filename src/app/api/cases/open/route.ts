...
export async function POST(request: Request) {
  try {
    ...
    if (!requestedCaseId) {
      return NextResponse.json({ error: "Не указан идентификатор кейса" }, { status: 400 });
    }
    ...
    const selectedCaseRecord = await prisma.case.findFirst({
      where: {
        OR: [{ id: requestedCaseId }, { slug: requestedCaseId }],
        isActive: true,
      },
      include: {
        drops: {
    ...
    if (!selectedCase || !selectedCaseRecord) {
      return NextResponse.json({ error: "Кейс не найден или отключён" }, { status: 404 });
    }

    if (!selectedCase.drops.length) {
      return NextResponse.json({ error: "У кейса нет доступных предметов" }, { status: 409 });
    }

    const baseProbabilityTotal = selectedCaseRecord.drops.reduce((total, drop) => total + drop.probability, 0);
    if (!validateChances(selectedCaseRecord.drops.map((drop) => drop.probability)) || !Number.isFinite(baseProbabilityTotal)) {
    ...
    if (user.balance < selectedCase.price) {
      return NextResponse.json(
        {
          error: "Недостаточно Z-Coin для открытия этого кейса.",
    ...
    const result: OpenResult = await prisma.$transaction(async (transaction) => {
      ...
      if (existing) {
        if (existing.userId !== user.id || existing.item?.caseId !== selectedCase.id) {
      ...
      const assignedDrop = assignment
        ? selectedCase.drops.find((candidate) => candidate.id === assignment.dropId)
        : null;
      const drop = assignedDrop ?? pickDrop(selectedCase.drops);

      if (assignment && !assignedDrop) {
        throw new Error("INVALID_FORCE_DROP_ASSIGNMENT");
      }

      const updatedUser = await transaction.user.update({
        where: { id: user.id },
        data: { balance: { decrement: selectedCase.price } },
      });
      ...
      await transaction.transaction.create({
        data: {
          userId: user.id,
          type: "CASE_OPEN",
          zCoinAmount: -selectedCase.price,
          status: "SUCCESS",
        },
      });

      if (assignment) {
        await transaction.forceDropAssignment.update({
    ...
    if (error instanceof Error && error.message === "INSUFFICIENT_BALANCE") {
      return NextResponse.json({ error: "Недостаточно Z-Coin для открытия этого кейса." }, { status: 400 });
    }

    if (error instanceof Error && error.message === "IDEMPOTENCY_CONFLICT") {
      return NextResponse.json({ error: "Ключ запроса уже использован для другой операции." }, { status: 409 });
    }
    ...
    if (error instanceof Error && error.message === "INVALID_FORCE_DROP_ASSIGNMENT") {
      return NextResponse.json({ error: "Ожидающий Force Drop больше не принадлежит этому кейсу." }, { status: 409 });
    }
