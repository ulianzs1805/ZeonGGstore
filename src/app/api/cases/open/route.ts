...
export async function POST(request: Request) {
  ...
  const selectedCaseRecord = await prisma.case.findFirst({ where: { OR: [{ id: requestedCaseId }, { slug: requestedCaseId }], isActive: true }, include: { drops: { orderBy: { createdAt: "asc" } } } });
  ...
  if (user.balance < selectedCase.price) return NextResponse.json({ error: "Недостаточно Z-Coin для открытия этого кейса.", casePrice: selectedCase.price, balance: user.balance }, { status: 400 });
  ...
  try {
    ...
    const assignment = await transaction.forceDropAssignment.findFirst({ where: { targetUserId: user.id, caseId: selectedCase.id, status: "PENDING" }, orderBy: { createdAt: "asc" } });
    ...
    if (assignment) {
      ...
      await transaction.auditLog.create({ data: { actorUserId: user.id, actorRole: user.role, actorAdminId: null, action: "FORCE_DROP_CONSUMED", targetType: "USER", targetId: user.id, metadata: JSON.stringify({ assignmentId: assignment.id, caseId: selectedCase.id, dropId: drop.id, inventoryItemId: item.id }), status: "SUCCESS" } });
    }
