...
export async function POST(request: Request) {
  try {
    ...
    if (preview) {
      const grant = await prisma.freeCaseGrant.findFirst({
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        const replayDrop = selectedCase.drops.find((d) => d.id === existing.item?.itemId);
        if (!replayDrop) throw new Error("IDEMPOTENCY_CONFLICT");
        return { balance: currentUser.balance, item: existing.item, drop: replayDrop, replay: true, freeOpen: existing.type === "CASE_OPEN_PROMO" };
      }

      const grant = await tx.freeCaseGrant.findFirst({
        where: { userId: user.id, caseId: selectedCase.id, consumedAt: null },
      ...
      if (!grant) {
        const charge = await tx.user.updateMany({
          where: { id: user.id, balance: { gte: selectedCase.price } },
          data: { balance: { decrement: selectedCase.price } },
        });
        if (charge.count !== 1) throw new Error("INSUFFICIENT_BALANCE");
      }

      const assignment = await tx.forceDropAssignment.findFirst({
        where: { targetUserId: user.id, caseId: selectedCase.id, status: "PENDING" },
        orderBy: { createdAt: "asc" },
      });
      const assignedDrop = assignment ? selectedCase.drops.find((d) => d.id === assignment.dropId) : null;
      ...
      await tx.operation.create({
        data: { userId: user.id, type: grant ? "CASE_OPEN_PROMO" : "CASE_OPEN", label: selectedCase.name, itemId: item.id, amount: grant ? 0 : -selectedCase.price, status: "SUCCESS", idempotencyKey },
      });
      await tx.transaction.create({
        data: { userId: user.id, type: grant ? "CASE_OPEN_PROMO" : "CASE_OPEN", zCoinAmount: grant ? 0 : -selectedCase.price, status: "SUCCESS" },
      });

      return { balance: freshUser.balance, item, drop, replay: false, freeOpen: Boolean(grant) };
    });
