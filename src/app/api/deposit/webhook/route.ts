...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (transaction.rubAmount !== Math.round(paidRub)) {
    ...
    if (payment.status === "succeeded" && payment.paid) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const operation = await tx.operation.findUnique({ where: { idempotencyKey: operationKey }, select: { id: true, label: true, status: true } });
        let metadata: { promoId?: string | null; bonusAmount?: number; amountRub?: number } = {};
        try { metadata = JSON.parse(operation?.label || "{}"); } catch {}
        const credit = current.zCoinAmount;
        ...
        if (metadata.promoId) {
          const promo = await tx.promoCode.findUnique({ where: { id: metadata.promoId }, select: { id: true, maxActivations: true, activationCount: true } });
          if (promo && promo.activationCount < promo.maxActivations) {
            await tx.promoCode.updateMany({ where: { id: promo.id, activationCount: { lt: promo.maxActivations } }, data: { activationCount: { increment: 1 } } });
            await tx.promoActivation.create({ data: { promoId: promo.id, userId: current.userId } }).catch(() => undefined);
          }
        }
