...
export async function GET(request: Request) {
  ...
  // ЮMoney удерживает комиссию с получателя: PC — 1% от суммы к получению,
  // AC — 3% от суммы к списанию. Поэтому gross-up нужен, чтобы Zeon получил ровно rubAmount.
  const desired = transaction.rubAmount;
  const sum = (paymentType === "PC" ? desired * 1.01 : desired / 0.97).toFixed(2);
