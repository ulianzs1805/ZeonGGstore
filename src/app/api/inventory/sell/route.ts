...
export async function POST(request: Request) {
  ...
  try {
    const result = await prisma.$transaction(async (transaction) => {
      const item = await transaction.inventoryItem.findFirst({ where: { id: inventoryItemId, userId: user.id, soldAt: null } });
      ...
      const updatedUser = await transaction.user.update({ where: { id: user.id }, data: { balance: { increment: creditAmount } } });
      const operation = await transaction.operation.create({ data: { userId: user.id, type: "ITEM_SALE", itemId: item.id, amount: creditAmount, status: "SUCCESS", createdAt: soldAt } });
    ...
    if (error instanceof Error && error.message === "ITEM_NOT_AVAILABLE") return NextResponse.json({ error: "Предмет уже продан или недоступен" }, { status: 409 });
    if (error instanceof Error && error.message === "WITHDRAWAL_EXISTS") return NextResponse.json({ error: "Сначала отмените активную заявку на вывод" }, { status: 409 });
    if (error instanceof Error && error.message === "INVALID_ITEM_PRICE") return NextResponse.json({ error: "У предмета некорректная цена" }, { status: 422 });
