...
// The weights are exact percentages because they add up to 100.
// The server still uses weightedPick, so the result is authoritative.
...
async function getLetterState(userId: string) {
  ...
  for (const spin of spins) {
    try {
      const data = JSON.parse(spin.label || "{}") as { metadata?: { slotId?: string } };
      const slot = data.metadata?.slotId;
      if (letterSlots.includes(slot as LetterSlot)) collected.add(slot as LetterSlot);
    } catch {}
  }
...
function getFreeCaseRoulette(cases: Awaited<ReturnType<typeof getCases>>) {
  ...
  indexes.add(0);
  indexes.add(Math.floor((sorted.length - 1) / 2));
  if (sorted.length > 3) indexes.add(sorted.length - 2);
  else indexes.add(sorted.length - 1);
  const selected = [...indexes].sort((a, b) => a - b).map((index) => sorted[index]);
  const unique = selected.filter((item, index, arr) => index === arr.findIndex((x) => x.id === item.id));
...
export async function GET() {
  ...
  const bonusInventory = await prisma.promoCode.findMany({ where: { ownerId: user.id, type: "DEPOSIT", inventorySaved: true, activationCount: { lt: 1 }, expiresAt: { gt: new Date() } }, select: { code: true, depositPercent: true, expiresAt: true }, orderBy: { createdAt: "desc" }, take: 10 });
  return NextResponse.json({ wheel, cases, depositRewards, bonusInventory, letterState: await getLetterState(user.id), word: "ZEONGG", letterSlots });
}
...
export async function POST(request: Request) {
  ...
  if (reward.type === "ZEON_SECRET") {
    ...
    const next = [...state.collected, slotId];
    ...
    if (completed && completionReward) {
      await prisma.$transaction(async (tx) => {
        await tx.user.update({ where: { id: user.id }, data: { balance: { increment: completionReward } } });
    ...
    const promo = await createUniquePromo(user.id, selected.amount);
    ...
    metadata = { bonusType: reward.type, percent: selected.amount, onDeposit: true, chanceWeight: selected.weight, promoCode: promo.code, promoType: "DEPOSIT", promoExpiresAt: promo.expiresAt.toISOString() };
    ...
    if (!rouletteCases?.length) return NextResponse.json({ error: "Не удалось подготовить бесплатные кейсы." }, { status: 409 });
    const selected = weightedPick(rouletteCases.map((item) => ({ item, weight: 1 / Math.max(1, item.price) })));
    caseId = selected.id;
    label = `Бесплатное открытие: ${selected.name}`;
    innerRoulette = { items: rouletteCases.map((item) => ({ key: item.id, title: item.name, subtitle: `${item.price} Z-Coin`, image: item.image })), selectedIndex: rouletteCases.findIndex((item) => item.id === selected.id), title: "Бесплатный кейс — выбираем кейс" };
    metadata = { bonusType: reward.type, caseId, caseName: selected.name, caseImage: selected.image, casePrice: selected.price, freeCaseSelection: "price_weighted" };
    ...
    const selected = weightedPick(pool.map((amount) => ({ item: amount, weight: amount >= 500 ? 1 : amount >= 250 ? 2 : 5 })));
    ...
    label = `Z-Coin Rain: +${selected} Z-Coin`;
    metadata = { bonusType: reward.type, amount: selected };
    await prisma.$transaction(async (tx) => {
