...
export async function GET() {
  ...
  return NextResponse.json({ wheel, cases, depositRewards, letterState: await getLetterState(user.id), word: "ZEONGG", letterSlots });
}
export async function POST(request: Request) {
  ...
  if (reward.type === "ZEON_SECRET") {
    ...
    if (completed && completionReward) await prisma.$transaction(async (tx) => {
