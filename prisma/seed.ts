...
async function main() {
	...
	for (const item of cases) {
		...
		if (item.slug === "furious" && existing) {
			await prisma.drop.deleteMany({ where: { caseId: existing.id, name: { in: [...legacyFuriousDropNames] } } });
		}
		...
		const currentCase = existing ?? await prisma.case.create({
			data: {
				...item,
		...
		if (item.slug === "furious" && existing && existingDrops && existingDrops.length > caseDrops.length) {
			await prisma.drop.deleteMany({ where: { caseId: currentCase.id, id: { in: existingDrops.slice(caseDrops.length).map((drop) => drop.id) } } });
		}

		console.log(`${existing ? "Synced" : "Created"} case: ${item.name}`);
	}
}
